...
@pytest.mark.parametrize(
    ...
    [
        deltakit_stim.CircuitInstruction("TICK", []),
        deltakit_stim.CircuitInstruction("QUBIT_COORDS", [0], (0, 1)),
    ],
)
...
#                          )
# def test_error_is_raised_if_qubit_is_acted_on_more_than_once_in_a_deltakit_stim_circuit(
#         gate_class):
...
#     times in a single deltakit_stim operation. It is possible to implement grouping for
#     two qubit gates but just hasn't been implemented so for now raise an error.
#     """
#     deltakit_stim_circuit = (deltakit_stim.Circuit("MPP X0 Z0")
#                     if issubclass(gate_class, sp.gates.MPP) else
#                     deltakit_stim.Circuit(f"{gate_class.deltakit_stim_string} 0 1 0 2"))
...
def test_multiple_gate_layers_are_created_for_gates_that_support_qubit_grouping(
    ...
    deltakit_stim_circuit = deltakit_stim.Circuit(
        f"{gate_class.deltakit_stim_string} 0 1 0 2"
    )
    assert len(sp.Circuit.from_deltakit_stim_circuit(deltakit_stim_circuit).layers) == 2
...
def test_moving_from_deltakit_stim_to_deltakit_circuit_to_deltakit_stim_preserves_coordinates_at_start_of_deltakit_stim_file():
    deltakit_stim_coordinates = "QUBIT_COORDS(1, 0) 0\nQUBIT_COORDS(0, 0) 1\n"
    instructions = deltakit_stim_coordinates + "X 1 \nCX 1 0\nM 0"
    deltakit_circuit_circuit = sp.Circuit.from_deltakit_stim_circuit(
        deltakit_stim.Circuit(instructions)
    )
    assert str(deltakit_circuit_circuit.as_deltakit_stim_circuit()).startswith(
        deltakit_stim_coordinates
    )


def test_moving_from_deltakit_stim_to_deltakit_circuit_to_deltakit_stim_preserves_deltakit_stim_ids():
    deltakit_stim_coordinates = "QUBIT_COORDS(1, 0) 0\nQUBIT_COORDS(0, 0) 1\n"
    instructions = deltakit_stim_coordinates + "X 1 \nCX 1 0\nM 0"
    deltakit_circuit_circuit = sp.Circuit.from_deltakit_stim_circuit(
        deltakit_stim.Circuit(instructions)
    ...
    assert (
        qubits[0].deltakit_stim_identifier == 0
        or qubits[1].deltakit_stim_identifier == 0
    )
    assert (
        qubits[0].deltakit_stim_identifier == 1
        or qubits[1].deltakit_stim_identifier == 1
    )
    assert len(deltakit_circuit_circuit.qubits) == 2


@pytest.mark.parametrize(
    "deltakit_stim_generated_circuit",
    [
        deltakit_stim.Circuit.generated(
            "repetition_code:memory", rounds=100, distance=9
        ),
        deltakit_stim.Circuit.generated(
        ...
        deltakit_stim.Circuit.generated(
            "surface_code:rotated_memory_x", rounds=100, distance=3
        ),
        deltakit_stim.Circuit.generated(
            "surface_code:rotated_memory_z", rounds=100, distance=3
        ),
    ],
)
def test_parsing_deltakit_stim_generated_circuits_does_not_raise_an_error(
    deltakit_stim_generated_circuit,
):
    assert sp.Circuit.from_deltakit_stim_circuit(deltakit_stim_generated_circuit)
