...
from deltakit_circuit._parse_deltakit_stim import (
    ...
    parse_deltakit_stim_gate_instruction,
)


def test_probability_is_added_on_measurement_gates():
    probability = 0.001
    layer = parse_deltakit_stim_gate_instruction(
        sp.gates.MZ, [deltakit_stim.GateTarget(0)], [probability], "", qubit_mapping={}
    )
    assert next(iter(layer.gates)).probability == probability


@pytest.mark.parametrize("non_pauli_target", [deltakit_stim.GateTarget(0)])
...
@pytest.mark.parametrize(
    ("deltakit_stim_circuit", "expected_gates"),
    [
        (
            deltakit_stim.Circuit("X 0 1 2 3 4"),
            [sp.gates.X(sp.Qubit(i)) for i in range(5)],
        ),
        (
        ...
        (deltakit_stim.Circuit("Y 0"), sp.gates.Y(sp.Qubit(0))),
        (deltakit_stim.Circuit("CNOT 0 1"), sp.gates.CX(sp.Qubit(0), sp.Qubit(1))),
        (
            deltakit_stim.Circuit(
                "QUBIT_COORDS(0, 0) 0 \nQUBIT_COORDS(0, 1) 1 \nCNOT 0 1"
            ),
        ...
        (
            deltakit_stim.Circuit("CNOT 0 1 2 3"),
            [
                sp.gates.CX(sp.Qubit(0), sp.Qubit(1)),
                sp.gates.CX(sp.Qubit(2), sp.Qubit(3)),
            ],
        ),
        (deltakit_stim.Circuit("RX 0"), sp.gates.RX(sp.Qubit(0))),
        (
            deltakit_stim.Circuit("RY 2 3 5"),
            [sp.gates.RY(sp.Qubit(i)) for i in (2, 3, 5)],
        ),
        (
            deltakit_stim.Circuit("MX 0 1 2 3"),
            [sp.gates.MX(sp.Qubit(i)) for i in range(4)],
        ),
        (
            deltakit_stim.Circuit(
                "QUBIT_COORDS(0, 0) 0 \nQUBIT_COORDS(0, 1) 1 \nMX 0 1"
        ...
        (
            deltakit_stim.Circuit("MRZ(0.1) 2 5"),
            [sp.gates.MRZ(sp.Qubit(i), 0.1) for i in (2, 5)],
        ),
        (
            deltakit_stim.Circuit("MRY !4 5"),
            [~sp.gates.MRY(sp.Qubit(4)), sp.gates.MRY(sp.Qubit(5))],
        ),
        (
            deltakit_stim.Circuit("MZ(0.4) 2 !0"),
            [sp.gates.MZ(sp.Qubit(2), 0.4), ~sp.gates.MZ(sp.Qubit(0), 0.4)],
        ),
        (deltakit_stim.Circuit("MPP Z0"), sp.gates.MPP(sp.PauliZ(sp.Qubit(0)))),
        (
            deltakit_stim.Circuit("MPP !Z5"),
            sp.gates.MPP(~sp.InvertiblePauliZ(sp.Qubit(5))),
        ),
        (
            deltakit_stim.Circuit("MPP(0.2) X0"),
            sp.gates.MPP(sp.PauliX(sp.Qubit(0)), 0.2),
        ),
        (
            deltakit_stim.Circuit("QUBIT_COORDS(0, 0) 0 \nMPP(0.2) X0"),
            sp.gates.MPP(sp.PauliX(sp.Coordinate(0, 0)), 0.2),
        ),
        (
            deltakit_stim.Circuit("MPP(0.1) !Y5"),
            sp.gates.MPP(~sp.InvertiblePauliY(sp.Qubit(5)), 0.1),
        ),
        (
            deltakit_stim.Circuit("MPP(0.2) X0 Z1"),
            [
                sp.gates.MPP(sp.PauliX(sp.Qubit(0)), 0.2),
        ...
        (
            deltakit_stim.Circuit("MPP X1*Y2"),
            sp.gates.MPP(
        ...
        (
            deltakit_stim.Circuit("MPP(0.01) Z1*X3"),
            sp.gates.MPP(
                sp.MeasurementPauliProduct(
        ...
        (
            deltakit_stim.Circuit("MPP !X4*Y2"),
            sp.gates.MPP(
                sp.MeasurementPauliProduct(
        ...
        (
            deltakit_stim.Circuit("MPP(0.02) Z5*!Y1"),
            sp.gates.MPP(
                sp.MeasurementPauliProduct(
        ...
        (
            deltakit_stim.Circuit("MPP X0 X1*X2"),
            [
        ...
        (
            deltakit_stim.Circuit("MPP Z0*Z1 Z2"),
            [
                sp.gates.MPP(
        ...
        (
            deltakit_stim.Circuit("MPP X0 Z1*Z2 Y3"),
            [
                sp.gates.MPP(sp.PauliX(sp.Qubit(0))),
                sp.gates.MPP(
        ...
        (
            deltakit_stim.Circuit("CXSWAP 0 1 2 3"),
            [
...
def test_parsing_deltakit_stim_circuit_with_single_gate_layer_returns_the_correct_deltakit_circuit_circuit(
    deltakit_stim_circuit, expected_gates
):
    expected_circuit = sp.Circuit(sp.GateLayer(expected_gates))
    assert (
        sp.Circuit.from_deltakit_stim_circuit(deltakit_stim_circuit) == expected_circuit
    )

    deltakit_stim_circuit = deltakit_stim.Circuit("CZSWAP 0 1 2 3")
    expected_gates = [
...
def test_parsing_controlled_gates_with_measurement_records_gives_expected_deltakit_circuit_circuit(
    ...
    expected_deltakit_circuit_circuit: sp.Circuit,
):
    assert (
        sp.Circuit.from_deltakit_stim_circuit(deltakit_stim_circuit)
        == expected_deltakit_circuit_circuit
    )
...
@pytest.mark.parametrize(
    ("deltakit_stim_circuit", "expected_circuit"),
    [
        (
            deltakit_stim.Circuit("X 1\nY 0"),
            sp.Circuit(
                [
        ...
        (
            deltakit_stim.Circuit("RZ 0 1 2 3 4\nM 0 1 2 3 4"),
            sp.Circuit(
                [
...
def test_parsing_multiple_gate_deltakit_stim_circuit_returns_correct_deltakit_circuit_circuit(
    deltakit_stim_circuit, expected_circuit
):
    assert (
        sp.Circuit.from_deltakit_stim_circuit(deltakit_stim_circuit) == expected_circuit
...
@pytest.mark.parametrize(
    ...
    [
        (
            deltakit_stim.Circuit("X 0 1 2 1 2"),
            sp.Circuit(
                [
        ...
        (
            deltakit_stim.Circuit("RZ 0 0 1 2 3"),
            sp.Circuit(
                [
