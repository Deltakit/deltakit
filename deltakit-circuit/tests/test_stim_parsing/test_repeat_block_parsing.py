...
def test_parsing_empty_repeat_block_gives_empty_circuit():
    assert sp.Circuit.from_deltakit_stim_circuit(
        deltakit_stim.Circuit("REPEAT 5 {}")
    ) == sp.Circuit([], 5)
...
def test_repeat_block_must_specified_in_deltakit_circuit_to_be_repeated_at_least_1_time(
    ...
    with pytest.raises(
        ValueError,
        match="Deltakit_Stim does not allow repeat blocks to be repeated less than 1 time. "
...
def test_parsing_repeat_block_with_one_repeat_flattens_the_circuit():
    ...
    """)
    assert sp.Circuit.from_deltakit_stim_circuit(deltakit_stim_circuit) == sp.Circuit(
        [
            sp.GateLayer(sp.gates.H(sp.Qubit(0))),
...
def test_parsing_deltakit_stim_circuit_which_is_just_a_repeat_block():
    deltakit_stim_circuit = deltakit_stim.Circuit("REPEAT 5 {\nX 0\n}")
    assert sp.Circuit.from_deltakit_stim_circuit(deltakit_stim_circuit) == sp.Circuit(
        sp.GateLayer(sp.gates.X(sp.Qubit(0))), iterations=5
    )


def test_parsing_deltakit_stim_circuit_with_repeat_block_creates_expected_deltakit_circuit_circuit():
    deltakit_stim_circuit = deltakit_stim.Circuit("H 0\nREPEAT 4 {\nCX 0 1\nCZ 1 2\n}")
    assert sp.Circuit.from_deltakit_stim_circuit(deltakit_stim_circuit) == sp.Circuit(
        [
            sp.GateLayer(sp.gates.H(sp.Qubit(0))),
            sp.Circuit(
...
def test_parsing_deltakit_stim_circuit_with_multiple_repeat_blocks():
    deltakit_stim_circuit = deltakit_stim.Circuit("""
        H 0 3
        REPEAT 5 {
            CX 0 1
...
def test_parsing_deltakit_stim_circuit_nested_repeat_blocks():
    deltakit_stim_circuit = deltakit_stim.Circuit("""
        REPEAT 5 {
            H 0
            REPEAT 3 {
...
def test_parsing_deltakit_stim_circuit_that_contains_nested_repeat_blocks():
    deltakit_stim_circuit = deltakit_stim.Circuit("""
        X 0 1
        REPEAT 5 {
            H 0
