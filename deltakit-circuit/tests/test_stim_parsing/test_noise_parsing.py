...
from deltakit_circuit._parse_deltakit_stim import parse_deltakit_stim_noise_instruction


def test_probability_is_added_on_noise_channels():
    probability = 0.01
    layer = parse_deltakit_stim_noise_instruction(
...
def test_error_is_raised_when_gate_class_is_passed_to_the_noise_parser():
    with pytest.raises(
        ...
        parse_deltakit_stim_noise_instruction(
            sp.gates.X, [0, 2, 3], [0.01], instruction_tag="", qubit_mapping={}
        )


@pytest.mark.parametrize(
    ("deltakit_stim_circuit", "expected_noise_channel"),
    [
        (
        ...
        (
            deltakit_stim.Circuit("Y_ERROR(0.4) 2"),
            sp.noise_channels.PauliYError(sp.Qubit(2), 0.4),
        ),
        (
            deltakit_stim.Circuit("Z_ERROR(0.3) 1"),
        ...
        (
            deltakit_stim.Circuit("DEPOLARIZE2(0.002) 0 4"),
        ...
        (
            deltakit_stim.Circuit("PAULI_CHANNEL_1(0.01, 0.02, 0.03) 3"),
        ...
        (
            deltakit_stim.Circuit("CORRELATED_ERROR(0.002) X0 Y3"),
        ...
        (
            deltakit_stim.Circuit("ELSE_CORRELATED_ERROR(0.001) Z2 X3"),
            sp.noise_channels.ElseCorrelatedError(
                sp.PauliProduct((sp.PauliZ(sp.Qubit(2)), sp.PauliX(sp.Qubit(3)))), 0.001
            ),
        ),
        # (deltakit_stim.Circuit("LEAKAGE(0.002) 0"),
        #  sp.noise_channels.Leakage(sp.Qubit(0), 0.002)),
        # (deltakit_stim.Circuit("RELAX(0.001) 0"),
        #  sp.noise_channels.Relax(sp.Qubit(0), 0.001)),
    ],
)
def test_parsing_single_noise_channel_deltakit_stim_circuit_returns_correct_deltakit_circuit_circuit(
    deltakit_stim_circuit, expected_noise_channel
):
...
@pytest.mark.parametrize(
    ...
    [
        (
            deltakit_stim.Circuit("X_ERROR(0.001) 0 1 2\nZ_ERROR(0.02) 0 1 2"),
            sp.Circuit(
                [
                    sp.NoiseLayer(
        ...
        # (
        #     deltakit_stim.Circuit("LEAKAGE(0.001) 0 1 2\nRELAX(0.02) 0 1 2"),
        #     sp.Circuit([
...
def test_parsing_multiple_noise_channel_deltakit_stim_circuit_returns_correct_deltakit_circuit_circuit(
    deltakit_stim_circuit, expected_circuit
):
    assert (
        sp.Circuit.from_deltakit_stim_circuit(deltakit_stim_circuit) == expected_circuit
